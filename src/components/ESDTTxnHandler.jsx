    ...
    useEffect(() => {
        (async () => {
            ...
            for (const scres of res['smartContractResults']) {
                ...
                const data = new Uint8Array(hexstr.match(/.{1,2}/g).map(by => parseInt(by, 16)));

                token_ident = decoder.decode(data);
            }

            const txu = elrd.unsignedSetESDTRoles(token_ident, new Address(sender), ["ESDTRoleNFTCreate"]);
